...
$(document).ready(function() {
    $(".a-ball-color").on('click', function(ev) {
        ...
        function gameLoop() {
            ...
            if(game.ball.inHole) {
                var nextLevel = game.currentLevel.number + 1
                var par = game.currentLevel.par
                var score = calculateScore(par, strokeCounter)
                domManipulator.insertScore(score)
                domManipulator.showFinishedHoleCard(par, strokeCounter)
                domManipulator.insertTableData(nextLevel, par, score)
            }
            requestAnimationFrame(gameLoop);
        };
        ...
        canvas.addEventListener('mousedown', function(event) {
            ...
            if(game.ball.inHole === false) {
                ...
                domManipulator.insertStroke(level, stroke);
            }

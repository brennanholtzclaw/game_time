...
// var golfBall = new Ball(150, 425, 6);
var golfBall = new Ball(50, 30, 6);
...
var obstacleCoords = [[[0, 200], [0, 10]]]

Ball.prototype.bounceCheck = function() {
  obstacleCoords.forEach(function (obstacle, index) {
    obstacle.forEach(function (coords, index) {
      if (index === 0) { // checking the X range
        if (this.x - this.radius >= coords[0] && this.x + this.radius <= coords[1]) {
        // if (this.x + this.radius >= coords[0] && this.x + this.radius <= coords[1] || this.x - this.radius >= coords[0] && this.x - this.radius <= coords[1]) {
        //// ^checking top left                    ^checking top right                    ^checking bottom left              ^ checking bottom right
          // debugger
          this.yDirection *= -1
          this.speed *= 0.8
        }
        ...
        if (this.y - this.radius >= coords[0] && this.y + this.radius <= coords[1]){
        // if (this.y + this.radius >= coords[0] && this.y + this.radius <= coords[1] || this.y - this.radius >= coords[0] && this.y - this.radius <= coords[1]){
          this.xDirection *= -1
          this.speed *= 0.8
        }
      }
